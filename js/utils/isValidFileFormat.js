function isValidFileFormat(fileName, allowedExtension) {
    if (typeof fileName !== 'string' ||
        fileName === '') {
        console.warn('ERROR: netinkamo tipo failo pavadinimas');
        return false;
    }
    ...
    if (fileParts[0].trim() === '') {
        console.warn('ERROR: failo pavadinimo pradzia (pries taska) turi buti ne tuscia');
        return false;
    }
    if (fileParts[1].trim() === '') {
        console.warn('ERROR: failo pavadinimo pabaiga (uz tasko) turi buti ne tuscia');
        return false;
    }
    if (!allowedExtension.includes(fileParts[1].trim())) {
        console.warn('ERROR: failo pletinys nera is tinkamo saraso');
        return false;
    }
